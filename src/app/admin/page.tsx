...
const getStatValueSize = (value: string) => {
  if (value.length > 14)
    return "text-[15px] sm:text-lg md:text-xl lg:text-[22px]";
  if (value.length > 10)
    return "text-base sm:text-xl md:text-2xl lg:text-[24px]";
  return "text-lg sm:text-2xl md:text-[24px]";
};
...
const StatCard = ({
  ...
  <motion.div
    ...
    className="group min-w-0 bg-white border border-[#f0f0f0] hover:border-orange-200 rounded-2xl p-4 sm:p-5 md:p-6 transition-all duration-300 hover:-translate-y-0.5"
  >
    <div className="flex items-start justify-between mb-3 sm:mb-4">
      <div className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center text-orange-500 text-[16px] sm:text-[18px] md:text-[20px] shrink-0">
        {icon}
      </div>
      <div className="w-1.5 h-1.5 rounded-full bg-orange-500/30 group-hover:bg-orange-500 transition-colors shrink-0" />
    </div>
    <p className="text-[9px] sm:text-[10px] font-bold tracking-widest sm:tracking-[0.16em] text-[#bbb] uppercase mb-1 truncate">
      {label}
    </p>
    <p
      title={value}
      className={`font-extrabold text-[#111] tabular-nums wrap-break-word leading-snug ${getStatValueSize(value)}`}
    >
      {value}
    </p>
  </motion.div>
...
export default function AdminDashboard() {
 ...
 const getDisplayPrice = (order: Order) => convertPrice(order.price);
  ...
  const { data: orders = [] } = useQuery<Order[]>({
    ...
    queryFn: () =>
      newRequest.get("/orders/all-completed").then((r) => r.data || []),
  });

  useEffect(() => {
    if (revenueData?.revenueData) {
      setSortedRevenueData(
        [...revenueData.revenueData].sort(
          (a, b) =>
            b.totalSellerRevenueConverted - a.totalSellerRevenueConverted,
        ),
      );
    }
  }, [revenueData]);
  ...
  const groupedRevenue = sortedRevenueData.slice(
    revenueStart,
    revenueStart + usersPerGroup,
  );
  ...
  const donutOption = {
    tooltip: {
      ...
      textStyle: { color: "#111", fontSize: 12 },
      formatter: (params: any) =>
        `${params.name}<br/>${formatPrice(currencySymbol, params.value)} (${params.percent}%)`,
    },
    series: [
      {
        type: "pie",
        ...
        label: {
          ...
          formatter: "{b}\n{d}%",
          fontSize: 11,
          color: "#111", // ← black instead of white
          fontWeight: "bold",
        },
        ...
        emphasis: {
          itemStyle: { shadowBlur: 20, shadowColor: "rgba(249,115,22,0.3)" },
        },
      },
    ],
  };
      ...
      <div className="px-5 md:px-8 py-7 space-y-5 max-w-300">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard
            ...
            value={formatPrice(currencySymbol, totalRevenue)}
          />
          <StatCard
            index={1}
            label="RMGC income (10%)"
            icon={<HiOutlineTrendingUp />}
            value={formatPrice(currencySymbol, totalRevenue * 0.1)}
          />
          <StatCard
            index={2}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-6 mb-6">
          ...
          >
            ...
            <PaginationBar
              ...
              onNext={() =>
                setRevenueGroup((p) =>
                  (p + 1) * usersPerGroup < sortedRevenueData.length
                    ? p + 1
                    : p,
                )
              }
          ...
          >
            ...
            <div className="space-y-3 mt-4">
              {[
                ...
                <div
                  key={item.label}
                  className="flex items-center justify-between py-2.5 border-b border-[#f7f7f7] last:border-0"
                >
        ...
        >
          ...
          {orders.length === 0 ? (
            ...
            <>
              {/* Table header */}
              <div className="hidden md:grid grid-cols-[1fr_1fr_1fr_150px_110px_100px] gap-4 px-4 py-2.5 border-b border-[#f5f5f5] bg-[#fafafa] rounded-xl mb-2">
                {[
                  "Order ID",
                  "Seller",
                  "Buyer",
              ...
              <div className="space-y-1">
                {orders.map((order, i) => (
                  <motion.div
                    ...
                    className="grid grid-cols-1 md:grid-cols-[1fr_1fr_1fr_150px_110px_100px] gap-4 items-center px-4 py-3.5 rounded-xl border border-transparent hover:border-[#f0f0f0] hover:bg-[#fafafa] transition-all"
                  >
                    ...
                    <span className="text-[13px] text-[#555] truncate">
                      {order.buyerName}
                    </span>
                    <span
              ...
              {groupedGigs.map((gig, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Body */}
                    <div className="p-4">
                      <p className="text-[13.5px] font-semibold text-[#111] truncate mb-1">
                        {gig.title}
                      </p>
                      <p className="text-[12px] text-[#aaa] line-clamp-2 leading-relaxed mb-3">
                        {gig.desc}
                      </p>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[12px] text-[#555] truncate max-w-25">
