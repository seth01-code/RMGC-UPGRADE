...
const GigSkeleton = () => (
  <div className="animate-pulse">
    <div className="w-full h-105 md:h-130 bg-[#111]" />
  ...
  return (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <IoMdStar
          key={i}
          className={`${sz} ${i < filled ? "text-amber-400" : "text-[#e0e0e0]"}`}
        />
      ))}
    </div>
  );
  ...
  return (
    <div className="mt-2 flex flex-col gap-0 overflow-hidden rounded-2xl border border-[#efefef]">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {portfolio.analyzedAt && (
                <span className="text-[10px] text-white/30 font-medium">
                  Verified{" "}
                  {new Date(portfolio.analyzedAt).toLocaleDateString("en-US", {
                    month: "short",
                    year: "numeric",
                  })}
                </span>
        ...
        <div className="px-6 py-5 border-b border-[#f5f5f5] bg-[#fafafa]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {portfolio.services!.map((s) => (
              <div
                key={s}
                className="flex items-center gap-2 text-[12.5px] text-[#444]"
              >
                ...
                <div
                  ...
                  className={`rounded-xl border transition-all duration-200 overflow-hidden ${
                    isOpen
                      ...
                      : "border-[#efefef] bg-[#fafafa] hover:border-[#e0e0e0]"
                  }`}
                >
                  <button
                  ...
                  >
                    {/* Number */}
                    <span
                      className={`shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-black transition-colors ${
                        isOpen
                    ...
                    <span className="flex-1 text-[13px] font-bold text-[#111] truncate">
                      {project.name}
                    </span>
                      ...
                      {project.technologies.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 pt-1">
                          {project.technologies.map((tech) => (
                            <span
                              key={tech}
                              className="px-2.5 py-1 bg-[#111] text-white text-[10.5px] font-semibold rounded-md"
                            >
                              {tech}
                            </span>
                          ))}
      ...
      {(hasIndustries || hasCerts) && (
        <div
          className={`grid gap-0 divide-x divide-[#f5f5f5] ${hasIndustries && hasCerts ? "grid-cols-2" : "grid-cols-1"} bg-[#fafafa]`}
        >
          {hasIndustries && (
            <div className="px-6 py-5">
              ...
              <div className="flex flex-wrap gap-2">
                {portfolio.industries!.map((ind) => (
                  <span
                    key={ind}
                    className="px-3 py-1.5 text-[11px] font-bold text-orange-600 bg-orange-50 border border-orange-100 rounded-lg"
                  >
                    {ind}
                  </span>
                ))}
              ...
              <div className="flex flex-col gap-2">
                {portfolio.certifications!.map((cert) => (
                  <div key={cert} className="flex items-start gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-orange-400 mt-1.25 shrink-0" />
                    <p className="text-[12px] text-[#444] font-medium leading-snug">
                      {cert}
                    </p>
                  </div>
                ))}
...
const GigPage: React.FC = () => {
  ...
  const hasPortfolio =
    portfolio && !Array.isArray(portfolio) && portfolio.status === "completed";
      ...
      <div className="flex justify-center items-center min-h-[60vh] px-4">
        <div className="max-w-sm w-full text-center py-16">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-orange-500 text-white text-[13px] font-bold rounded-xl hover:bg-orange-600 transition"
          >
            Refresh
          </button>
        </div>
      ...
      <div className="relative w-full bg-[#0a0a0a] select-none overflow-hidden">
        {allMedia.length > 0 ? (
          <>
            <div
              ref={prevRef}
              className="absolute left-5 top-1/2 -translate-y-1/2 z-20 w-10 h-10 flex items-center justify-center bg-black/50 backdrop-blur-md border border-white/10 rounded-full cursor-pointer hover:bg-black/70 transition text-white text-xl font-light leading-none"
            >
              ‹
            </div>
            <div
              ref={nextRef}
              className="absolute right-5 top-1/2 -translate-y-1/2 z-20 w-10 h-10 flex items-center justify-center bg-black/50 backdrop-blur-md border border-white/10 rounded-full cursor-pointer hover:bg-black/70 transition text-white text-xl font-light leading-none"
            >
              ›
            </div>
            <div className="absolute bottom-0 left-0 right-0 h-32 bg-linear-to-t from-black/60 to-transparent z-10 pointer-events-none" />
            <div className="absolute bottom-5 right-5 z-20 bg-orange-500 text-white px-4 py-2.5 rounded-2xl shadow-xl shadow-orange-900/30">
              <p className="text-[9px] font-bold tracking-[0.2em] uppercase opacity-80 mb-0.5">
                Starting at
              </p>
            ...
            <div className="absolute bottom-5 left-5 z-20">
              <Link
                href={`/allgigs?cat=${data.cat}`}
                className="inline-flex items-center gap-1.5 bg-white/10 backdrop-blur-md border border-white/15 text-white text-[11px] font-semibold px-3.5 py-2 rounded-full hover:bg-white/20 transition"
              >
                {data.cat}
                <HiArrowRight className="text-[10px] opacity-70" />
              </Link>
            </div>
            <Swiper
              spaceBetween={0}
              ...
              pagination={{
                clickable: true,
                bulletActiveClass:
                  "!bg-orange-500 !opacity-100 !w-4 !rounded-sm",
                bulletClass:
                  "swiper-pagination-bullet !bg-white !opacity-30 transition-all duration-200",
              }}
              onBeforeInit={(swiper) => {
              ...
              className="w-full h-95 md:h-130 [&_.swiper-wrapper]:h-full [&_.swiper-slide]:h-full [&_.swiper-pagination]:bottom-14"
            >
          ...
          <div className="w-full h-60 flex items-center justify-center text-white/20 text-sm tracking-wider">
            No media provided
          </div>
        )}
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12 py-10 grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 flex flex-col gap-7">
            ...
            <div className="flex flex-col gap-4">
              <h1 className="text-[27px] md:text-[33px] font-black text-[#0a0a0a] leading-[1.2] tracking-tight">
                {data.title}
              </h1>
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <div className="relative shrink-0 w-10 h-10">
                    <Image
                      ...
                      fill
                      className="rounded-full object-cover ring-2 ring-orange-200 ring-offset-1"
                      sizes="40px"
                      unoptimized
                    />
                  ...
                  <div>
                    <p className="text-[13.5px] font-bold text-[#111] leading-none">
                      {dataUser.username}
                    </p>
                    {dataUser.country && (
            ...
            {activeTab === "about" && (
              <div className="flex flex-col gap-5">
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#f0f0f0]">
                  <p className="text-[13.5px] text-[#555] leading-[1.8] whitespace-pre-line wrap-break-word">
                    {data.desc}
                  </p>
                </div>
                {data.features.length > 0 && (
                  <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#f0f0f0]">
                    <p className="text-[9.5px] font-black tracking-[0.2em] text-orange-500 uppercase mb-4">
                      What&apos;s included
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {data.features.map((f, i) => (
                        <div
                          key={i}
                          className="flex items-start gap-2.5 text-[13px] text-[#444]"
                        >
                          <FaCheckDouble className="text-orange-500 mt-0.75 shrink-0 text-[10px]" />
                          <span>{f}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex flex-col gap-1 min-w-0 pt-1">
                        ...
                        {hasPortfolio && portfolio.headline ? (
                          <p className="text-[12px] text-white/60 leading-snug mt-0.5">
                            {portfolio.headline}
                          </p>
                        ) : dataUser.desc ? (
                          <p className="text-[12px] text-white/60 leading-snug mt-0.5 line-clamp-2">
                            {dataUser.desc}
                          </p>
                        ) : null}
                      </div>
                  ...
                  <div className="mx-5 -mt-8 mb-1 relative z-10">
                    <div className="bg-white rounded-2xl border border-[#efefef] shadow-md grid grid-cols-2 sm:grid-cols-4 divide-x divide-[#f5f5f5]">
                      {[
                        ...
                        {
                          icon: <FaLanguage className="text-[11px]" />,
                          label: "Languages",
                          value: dataUser.languages?.join(", ") || "—",
                        },
                        {
                          icon: <FaBriefcase className="text-[11px]" />,
                          label: "Experience",
                        ...
                        >
                          <p className="text-[9px] font-black tracking-[0.15em] text-[#ccc] uppercase flex items-center gap-1">
                            {stat.icon} {stat.label}
                          </p>
                          <p className="text-[12.5px] font-bold text-[#222] truncate">
                            {stat.value}
                          </p>
                        </div>
                      ))}
          ...
          <div className="lg:sticky lg:top-28 h-fit flex flex-col gap-3">
            <div className="bg-white rounded-2xl shadow-lg border border-[#ececec] overflow-hidden">
              <div className="bg-[#0a0a0a] px-6 py-5">
                <p className="text-[9px] font-bold tracking-[0.25em] text-orange-400 uppercase mb-1">
                  {data.shortTitle}
                </p>
              ...
              <div className="px-6 py-5 flex flex-col gap-5">
                ...
                {data.features.length > 0 && (
                  <div className="flex flex-col gap-2.5">
                    {data.features.map((f, i) => (
                      <div
                        key={i}
                        className="flex items-start gap-2 text-[12.5px] text-[#555]"
                      >
                        <FaCheckDouble className="text-orange-500 mt-0.75 shrink-0 text-[9px]" />
                        <span>{f}</span>
                      </div>
                    ))}
                  </div>
                ...
                {!currentUser?.isSeller && (
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 text-[12.5px] font-bold text-[#999] hover:text-orange-500 transition py-0.5"
                  >
                    <FaRegCalendarAlt className="text-[13px]" /> Schedule a
                    meeting
                  </button>
                )}
              </div>
