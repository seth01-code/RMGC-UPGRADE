  ...
  try {
    ...
    if (paymentMethod === "paystack" && status === 400) {
      toast.error("Payment unavailable in your region", {
        description: "Paystack only supports Nigeria and the USA. Please use Flutterwave for international payments.",
        duration: 6000,
      });
      setErrorMessage("Paystack is not available in your country. Please use Flutterwave.");
    } else if (paymentMethod === "flutterwave" && status === 400) {
      toast.error("Flutterwave unavailable for your country", {
        description: "Nigerian users must complete payment via Paystack. Please select Paystack above.",
        duration: 6000,
      });
      setErrorMessage("Please use Paystack to complete your payment.");
    } else {
      const message = err.response?.data?.message || err.message || t("unknownError");
